                        ...
                        <div class="w-auto p-3">
                            ...
                            <div class="panel-heading-admin">
                                @foreach($dados as $dado)
                                    <div class="col-12">
                                        <h5>Resultado Eleição Governador 1º Turno <strong>{{$dado->nm_urna_candidato}}</strong> </h5>
                                        <h5>Total de Votos: <strong>{{number_format((float)$dado->total_votos,0, '', '.')}}</strong></h5>
                                    </div>
                            </div>
                            <div class="panel-body">
                                <div class="form-search">
                                    <form action="{{route('show_governo', ['cand' => $dado->sq_candidato ])}}" method="get">
                                ...
                                <div class="row btn-new-reset">
                                    <div class="btn-hero">
                                        <p><a href="{{route('governo')}}" class="btn btn-success btn-assinar">Voltar</a></p>
                                        <p><a href="{{route('show_governo', ['cand' => $dado->sq_candidato ])}}" class="btn btn-primary btn-assinar">Limpar</a></p>
                                            @break
                                        @endforeach
                                    </div>
                                </div>
                                   ...
                                   <table class="table table-striped">
                                       ...
                                       @foreach($depests as $depest)
                                           <tr>
                                               ...
                                               <td style="text-align: center; color: #0a58f6;"><a href="{{route('show_munic_gov', ['munic' => $depest->cod_municipio])}}"><i class="fas fa-eye"></i></a></td>
                                           </tr>
                                       @endforeach
                                   </table>
                                </div>
