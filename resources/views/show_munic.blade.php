                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="w-auto p-3">
                            <div class="col-12">
                                <h5 style="text-align: center">Eleição Deputado Estadual <strong>{{$detalhe->nm_municipio}} - Comparecimento: {{number_format((float)($detalhe->aptos - $detalhe->abstencoes),0, '', '.')}}</strong></h5>
                            </div>
                            ...
                            <div class="panel-body">
                                <div class="form-search">
                                    <form action="{{route('show_munic', ['munic' => $munic])}}" method="get">
                                ...
                                <div class="row btn-new-reset">
                                    <div class="btn-hero">
                                        ...
                                        <p><a href="{{route('show_munic', ['munic' => $munic])}}" class="btn btn-primary btn-assinar">Limpar</a></p>
                                    </div>
                                </div>
                                   ...
                                   <table class="table table-striped">
                                       ...
                                       @foreach($depests as $depest)
                                           <tr>
                                               ...
                                               <td style="text-align: center; color: #0a58f6;"><a href="{{route('show_depest', ['cand' => $depest->sq_candidato])}}"><i class="fas fa-eye"></i></a></td>
                                           </tr>
                                       @endforeach
                                       @foreach($suplentes as $suplente)
                                           <tr>
                                               <td>{{$suplente->nm_urna_candidato}} ({{$suplente->sg_partido}})</td>
                                               <td>{{$suplente->ds_composicao_coligacao}}</td>
                                               <td>{{$suplente->ds_sit_tot_turno}}</td>
                                               <td style="text-align: right;">{{number_format((float)$suplente->total_votos,0, '', '.')}}</td>
                                               <td style="text-align: center; color: #0a58f6;"><a href="{{route('show_depest', ['cand' => $suplente->sq_candidato])}}"><i class="fas fa-eye"></i></a></td>
                                           </tr>
                                       @endforeach
                                       @foreach($naoeleitos as $naoeleito)
                                           <tr>
                                               <td>{{$naoeleito->nm_urna_candidato}} ({{$naoeleito->sg_partido}})</td>
                                               <td>{{$naoeleito->ds_composicao_coligacao}}</td>
                                               <td>{{$naoeleito->ds_sit_tot_turno}}</td>
                                               <td style="text-align: right;">{{number_format((float)$naoeleito->total_votos,0, '', '.')}}</td>
                                               <td style="text-align: center; color: #0a58f6;"><a href="{{route('show_depest', ['cand' => $naoeleito->sq_candidato])}}"><i class="fas fa-eye"></i></a></td>
                                           </tr>
                                       @endforeach
                                   </table>
                                </div>
